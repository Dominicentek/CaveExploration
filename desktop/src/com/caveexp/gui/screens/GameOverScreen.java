...
public class GameOverScreen extends Screen {
    ...
    public void init() {
        ...
        addComponent(GUIButtonComponent.build(-128, y, 256, 32, "$nNew Game", () -> {
            Game.newGame(RNG.random(), false);
            Main.screenStack.pop();
        }).anchored(PositionAnchor.CENTER));
        addComponent(GUIButtonComponent.build(-128, y + 37, 256, 32, "$nSandbox Mode", () -> {
            Game.newGame(RNG.random(), true);
            Main.screenStack.pop();
        }).anchored(PositionAnchor.CENTER));
        addComponent(GUIButtonComponent.build(-128, y + 37 * 2, 256, 32, "$nLoad Game", () -> {
            new Thread(() -> {
        ...
        addComponent(GUIButtonComponent.build(-128, y + 37 * 3, 256, 32, "$nQuit", () -> {
            Main.screenStack.pop();
